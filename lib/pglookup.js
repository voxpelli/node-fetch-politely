...
/**
 * @typedef PolitePGLookupOptions
 * @property {typeof console.log} log
 * @property {import('bunyan-adaptor').BunyanLite} logger
 * @property {*} knex
 * @property {number} [concurrentReleases]
 * @property {boolean} [onlyDeduplicateMessages]
 * @property {number} [purgeWindow]
 * @property {number} [releasesPerBatch]
 * @property {number} [throttleDuration]
 */

class PolitePGLookup {
  /**
   * @param {PolitePGLookupOptions} options
   */
  constructor (options) {
  ...
  async _checkForReleasables (retry) {
    const knex = this.knex;
    ...
    return this._purgeUnthrottledHosts()
      .then(async () => {
        ...
        return this.stopped ? false : knex.transaction(async trx => {
  ...
  async _watchForReleasables () {
    ...
    const foundUrls = await this._checkForReleasables().catch(err => {
  ...
  async _reserveSlot (hostname) {
    ...
    try {
      const result = await knex('polite_hosts')
        .insert(
      ...
      return result.rowCount !== 0; // Did we manage to reserve it?
    } catch (err) {
