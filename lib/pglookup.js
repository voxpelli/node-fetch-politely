...
PolitePGLookup.prototype._purgeUnthrottledHosts = function () {
  ...
  this.debug('Purging expired hosts');
  var knex = this.knex;
...
PolitePGLookup.prototype._checkForReleasables = function (retry) {
  ...
  return this._purgeUnthrottledHosts()
    ...
    .then(function (result) {
      ...
      var hostnames = _.map(result.rows, function (row) {
        return row.hostname;
      });
      ...
      .then(function (result) {
          ...
          var ids = _.map(result.rows, function (row) {
            return row.id;
          });
...
PolitePGLookup.prototype._watchForReleasables = function () {
  ...
  if (this.stopped) {
    this.debug('Not going to watch due to stopping');
    return;
  }
    ...
    .then(function (foundUrls) {
      if (foundUrls >= that.options.releasesPerBatch) {
        if (that.concurrentReleases < that.options.concurrentReleases) {
          that.debug('Found', foundUrls, 'URL:s. Adding new release check:', that.concurrentReleases + 1);
          that._watchForReleasables();
        } else {
          that.debug('Reached max concurrent releases of', that.options.concurrentReleases);
        ...
        that.debug('Retrying in', delay, 'ms');
