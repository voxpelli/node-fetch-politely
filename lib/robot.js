...
class PoliteRobot {
  ...
  _fetch (url) {
    const robotUrl = urlModule.resolve(url, '/robots.txt');
  ...
  allowed (url) {
    return this._fetch(url).then(robotContent => {
      if (robotContent === false) {
        ...
        return parser.canFetchSync(this.options.userAgent, urlModule.parse(url).pathname);
        // TODO: Make use of crawldelay as well!
      }
    }).then(access => {
      this.log('We', access ? 'are' : 'are NOT', 'allowed to fetch', urlModule.parse(url).hostname + '' + urlModule.parse(url).pathname);
      return access;
    });
  }
}
